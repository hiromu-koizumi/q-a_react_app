       ...
       return (
        <div className="good-button-wrap">
            ...
            {/* 　　どのハートが押されたか識別するためにIdが必要。詳細ページでは、回答は同じquestionIdを取得しているためanswerIdで識別する必要がある。 */}
                <input className="heart-input" onClick={this.onClick} id={this.props.postData.answerId ? this.props.postData.answerId : this.props.postData.questionId} type="checkbox"/>
                <label className="heart-label" htmlFor={this.props.postData.answerId ? this.props.postData.answerId : this.props.postData.questionId}><i className="ui heart icon"></i></label>
